{{--Header CSS--}}
<link href="{{ asset('custom/css/header/base/light.css') }}" rel="stylesheet" type="text/css" />
<link href="{{ asset('custom/css/header/menu/light.css') }}" rel="stylesheet" type="text/css" />
{{--End Header CSS--}}
<link rel="stylesheet" href="{{ asset('custom/css/global.css') }}">
<link rel="stylesheet" href="{{ asset('lib/line-awesome/dist/line-awesome/css/line-awesome.min.css') }}">
<link href="{{ asset('custom/css/style.bundle.css') }}" rel="stylesheet" type="text/css" />
<link rel="stylesheet" href="{{ asset('custom/css/style.css') }}">
<link rel="stylesheet" href="{{ asset('lib/perfect-scrollbar/css/perfect-scrollbar.css') }}">
<link rel="stylesheet" href="{{ asset('lib/font-awesome/css/font-awesome.min.css') }}">
<link rel="stylesheet" href="{{ asset('lib/sweetalert2/dist/sweetalert2.min.js') }}">
